...
class Dual_MTLR (nn.Module):
    def __init__(self, 
        ...
        self.radiomics = nn.Sequential (# block 1
                                        ...
                                        # block 3
                                        #conv_3d_block (512, 512, kernel_size=3),
                                        #conv_3d_block (512, 512, kernel_size=3),
                                        # nn.MaxPool3d(kernel_size=2, stride=2),
...
class Image_MTLR (nn.Module):
    def __init__(self, 
        ...
        if n_dense <= 0:
            self.mtlr = MTLR(512, 29, num_events=num_events)
        else: 
            fc_layers = [[nn.Linear(512, 512 * dense_factor), 
                          nn.BatchNorm1d(512 * dense_factor),
                          nn.ReLU(inplace=True), 
                          nn.Dropout(dropout)]]   
